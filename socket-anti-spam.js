...
exports.addSpam = function (socket) {
  ...
  authenticate(socket, function (data) {
    ...
    if (data.score >= options.kickThreshold && !data.kicked) {
      data.kicked = true
...
function authenticate(socket, cb) {
  ...
  if (data.banned) {
    let banned = false
    const bannedUntil = data.bannedUntil.diff(moment(), "seconds")
    if (heartbeats[socket.id]) {
      clearHeart(socket)
    }
    if (bannedUntil >= 1) {
      banned = true
      socket.bannedUntil = bannedUntil
    }
    data.banned = banned
